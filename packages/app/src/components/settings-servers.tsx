...
import { DialogWslServer } from "./dialog-wsl-server"
  ...
  return (
    <div class="flex flex-col h-full overflow-y-auto no-scrollbar px-4 pb-10 sm:px-10 sm:pb-10">
      <div class="flex flex-col flex-1 min-h-0 max-w-[720px]">
        <Switch>
          <Match when={controller.isAddWslMode()}>
            <div class="flex flex-1 min-h-0 flex-col gap-4 pt-6">
              <div class="text-16-medium text-text-strong">{controller.formTitle()}</div>
              <DialogWslServer onAdded={controller.handleAddedWsl} />
            </div>
          </Match>
          <Match when={controller.isFormMode()}>
            <div class="flex flex-1 min-h-0 flex-col gap-4 pt-6">
              <div class="text-16-medium text-text-strong">{controller.formTitle()}</div>
